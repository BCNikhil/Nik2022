...
class Distance:
    ...
    def findCollar(self):
        leftEyeOuter = self.keypoints[3]
        rightEyeOuter = self.keypoints[6]
        neckDistPixel = sqrt(
            (leftEyeOuter[1]-rightEyeOuter[1])**2+(leftEyeOuter[2]-rightEyeOuter[2])**2)
        neckDist = (self.distanceBody * neckDistPixel)/self.focal
        neckRadius = neckDist/2

        # formula for finding collar distance
        self.collarDist = round((2*(22/7)*neckRadius), 2)*1.6
    ...
    def findInseam(self):
        ...
        leftKnee = self.keypoints[25]
        rightKnee = self.keypoints[26]
        leftHeel = self.keypoints[29]
        rightHeel = self.keypoints[30]
        rightInseamDistTop = sqrt(
            (rightHip[1]-rightKnee[1])**2+(rightHip[2]-rightKnee[2])**2)
        leftInseamDistTop = sqrt(
            (leftHip[1]-leftKnee[1])**2+(leftHip[2]-leftKnee[2])**2)

        rightInseamDistBot = sqrt(
            (rightKnee[1]-rightHeel[1])**2+(rightKnee[2]-rightHeel[2])**2)
        leftInseamDistBot = sqrt(
            (leftKnee[1]-leftHeel[1])**2+(leftHeel[2]-leftKnee[2])**2)

        rightInseamDist = rightInseamDistBot + rightInseamDistTop
        leftInseamDist = leftInseamDistBot + leftInseamDistTop
    ...
    def findWaist(self):
        ...
        waistDist = 2*(22/7)*sqrt((waistRadius**2+(waistRadius/2)**2)/2)*1.1
    ...
    def findHip(self):
        ...
        hipDistPixel = sqrt(
            (leftHip[0]-rightHip[0])**2+(leftHip[1]-rightHip[1])**2)
        hipRadiusPixel = hipDistPixel/2
        hipRadius = (self.distanceBody * hipRadiusPixel)/self.focal
        hipDist = 2*(22/7)*sqrt((hipRadius**2+(hipRadius/2)**2)/2)*2
    ...
    def findFullSleeve(self):
        ...
        rightSleeveDistTop = sqrt(
            (rightElbow[1]-rightShoulder[1])**2+(rightElbow[2]-rightShoulder[2])**2)
        leftSleeveDistTop = sqrt(
            (leftElbow[1]-leftShoulder[1])**2+(leftElbow[2]-leftShoulder[2])**2)
    ...
    def findChest(self):
        ...
        chestDist = 2*(22/7)*sqrt((chestRadius**2+(chestRadius/3)**2)/2)*1.4
